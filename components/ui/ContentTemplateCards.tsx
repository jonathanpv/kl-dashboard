...
export const ContentTemplateCardVariant5 = ({ open, id, videoSrc, title, thumbnailSrc }: { open: () => void; id: string, videoSrc: string, title: string, thumbnailSrc: string }) => {
  ...
  return (
    <motion.div
      ...
      className="cursor-pointer relative group flex w-full flex-col items-center gap-[6px] rounded-[13px] bg-background hover:scale-[1.02] transition-transform duration-300"
      layoutId={`card-container-${id}`}
    >
      {/* MainContent */}
      <motion.div
        className="relative flex aspect-[2/3] flex-col justify-center items-center shrink-0 self-stretch rounded-[12px] bg-background overflow-hidden"
